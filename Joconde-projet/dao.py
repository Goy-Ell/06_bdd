...
def get_all_auteur():
    ...
    mySql_get_all_auteur = 'SELECT id, nom FROM artiste'
    try:
...
def get_auteur_id_by_name2(aut):
    dbc = Database(USER, PASSWORD, HOST, DB_NAME)
    mySql_get_auteur = "SELECT id FROM artiste WHERE nom LIKE CONCAT('%', %s,'%')"
    try:
...
def insert_many_art_oeuvs(art_oeuvs):
    ...
    for aut, id_oeuvre in tqdm(art_oeuvs):
        ...
        try:
            ...
            print(f'{aut} not found in art_oeuv list' , e)
            continue
        try:
            ...
            print(
                f"error at insert_many_art_oeuvs({id_aut, id_oeuvre}) // ",  e)
            # TODO add to log file
            continue
    dbc.close()
...
def research_by_artiste2(artiste):
    dbc = Database(USER, PASSWORD, HOST, DB_NAME)
    mySql_research = f"""SELECT m.nom, m.ville, m.geoloc, a.nom, o.nom, o.domaine, o.musee
                        FROM musee m 
                        JOIN oeuvre o ON  o.musee = m.id
    ...
    response = dbc.query(mySql_research,artiste)
    df = pd.DataFrame(response,columns=['m_nom', 'ville', 'geoloc', 'a_nom', 'o_nom', 'domaine', 'musee'])
